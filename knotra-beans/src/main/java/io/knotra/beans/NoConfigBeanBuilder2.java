...
/** 无配置 Bean 定义的 2 依赖阶段；不可变，每次调用返回新实例。 */
public final class NoConfigBeanBuilder2<D1, D2> {

    private final String componentId;
    private final BeanDependency<D1> first;
    private final BeanDependency<D2> second;

    NoConfigBeanBuilder2(String componentId, BeanDependency<D1> first, BeanDependency<D2> second) {
        this.componentId = componentId;
        this.first = first;
        this.second = second;
    }

    public <D3> NoConfigBeanBuilder3<D1, D2, D3> with(BeanDependency<D3> third) {
        Objects.requireNonNull(third, "third");
        return new NoConfigBeanBuilder3<>(componentId, first, second, third);
    }

    public <D3, D4> NoConfigBeanBuilder4<D1, D2, D3, D4> with(
    ...
    public <D3, D4, D5> NoConfigBeanBuilder5<D1, D2, D3, D4, D5> with(
            BeanDependency<D3> third,
            BeanDependency<D4> fourth,
    ...
    public <T> BeanOutputStage<NoConfig, T> create(BeanCreator2<D1, D2, T> creator) {
