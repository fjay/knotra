...
结算服务只依赖 `DiscountStrategy` 接口。通过注入 **`dynamicProxyRequired` 动态代理**，底层策略发生热替换或升级时，结算服务自身**保持在线且无需重启**：
...
import com.example.promotion.contract.PromotionContracts;
...
public final class CheckoutService {
    private final DiscountStrategy discountStrategy; // 动态代理对象
...
import com.example.promotion.core.CheckoutService;
    ...
    public static void main(String[] args) {
        try (KnotraRuntime runtime = KnotraRuntime.create()) {
            ...
            CheckoutService checkoutService = new CheckoutService(
                    runtime.root().view().require(PromotionContracts.DISCOUNT_STRATEGY));
            ...
            // 4. 运行时热替换为 V2 策略（满 200 减 50），应用不重启
            ...
            // 5. 再次处理订单（无缝命中 V2 策略，CheckoutService 实例未发生任何重启）
