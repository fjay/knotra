...
- `Registration<T>` names one committed generation of a capability value.
- Each publication operation returns its own `PublicationChange<T>` and the settlement for that exact change.
- `PublicationChange.registration()` exposes a registration only for PUBLISH and UPDATE. An UNPUBLISH change expresses removal only: `registration()` is null and the runtime never exposes the removed, stale registration through that change.
...
- `Publication.currentRegistration()` returns an empty `Optional` when the slot is unpublished or displaced; it never returns null.
...
- Settlement reports are operation-scoped. `hasFailedMounts()` and `allActive()` are explicit predicates; an empty affected set has no failed mounts but is not all-active.
## Consequences
