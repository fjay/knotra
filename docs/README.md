...
虽然业界存在 Spring 动态刷新、PF4J、OSGi 等方案，但在以下维度仍有明显局限：
...
| **ClassLoader 回收** | 不涉及类卸载 | 极易发生类加载器引用泄漏导致 OOM | 机制沉重，学习曲线陡峭 | **纯数据快照与有界诊断**：框架不持有类引用，保证彻底 GC |
...
- [快速开始](quick-start.md)：从依赖引入、契约定义、组件装配到平滑热替换的完整路径
- [Beans 装配](beans-guide.md)：Beans DSL、6 种依赖注入模式、生命周期管理与编译期注解处理器
- [Spring 集成](spring-guide.md)：Spring 子容器隔离挂载与 Spring Boot 宿主单例动态桥
- [插件工程](plugin-guide.md)：三层工程结构、受控工厂导出、声明式调和与 ClassLoader 防泄漏红线
- [运行时内核](runtime-kernel.md)：领域模型、API 三层体系、高级结构事务、Context 树与事件总线
- [生产实践与排障](production-practice.md)：执行边界、超时预算、优雅停机、挂起诊断、测试规范与 FAQ
- [实战案例](case-sample.md)：动态营销折扣引擎、一致性租约、批量结算与多租户灰度隔离
