...
### 真实生产中的痛点：
1. **上海仓与深圳仓规则不同**：需要按仓库（多租户/Context）隔离路由规则。
2. **规则每周热升级**：升级为 V2 规则时，应用不能重启。
3. **在途包裹不能丢**：已经分发给旧规则处理中的包裹，必须等待其计算完毕；不能在方法执行中途把旧规则实例暴力销毁。
4. **插件卸载 ClassLoader 必须回收**：长期多次热升级，Metaspace 内存不能增长。
...
2. **零停机平滑热更**：通过 `dynamicProxyRequired` 动态代理，消费方不需要重启，每次调用自动穿透到最新 Provider；
3. **在途任务安全保护**：卸载 V1 插件时，通过租约排空（Drain）机制，保证已进入 V1 的包裹计算完毕后再注销 ClassLoader。
