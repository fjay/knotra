 ...
 */
public final class CompositeFactoryResolver implements ComponentFactoryResolver {

    private final List<ComponentFactoryResolver> resolvers;

    private CompositeFactoryResolver(List<ComponentFactoryResolver> resolvers) {
        this.resolvers = List.copyOf(resolvers);
    }

    /** 按参数顺序组合解析器，靠前的解析器优先。 */
    public static ComponentFactoryResolver of(ComponentFactoryResolver... resolvers) {
        return new CompositeFactoryResolver(List.of(resolvers));
    }

    /** 按列表顺序组合解析器，靠前的解析器优先。 */
    public static ComponentFactoryResolver of(List<ComponentFactoryResolver> resolvers) {
        return new CompositeFactoryResolver(resolvers);
    }
    ...
    public Optional<ResolvedFactory> resolve(FactoryRef ref) {
        for (ComponentFactoryResolver resolver : resolvers) {
            Optional<ResolvedFactory> definition = resolver.resolve(ref);
